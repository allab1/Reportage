...
if( !$("#tfr").val() && !$("#ten").val() && !$("#tes").val())
    {
        $("#errorter").text("Au moins Une langue doit être saisit");
    }
else 
    {
      $("#errorter").text("");
...
if(!$("#commanditaire1").val() || !$("#reporter").val() || !$("#start").val() || !$("#end").val())
   $("#errordesc").text("Il vous manque un champ *");
  
else if( !$("#fr").val() && !$("#en").val() && !$("#es").val())
    
        $("#errordesc").text("Au moins Une langue doit être saisit *");
...
$("#okmodal1").click(function()
{
      ...
      $("#"+f).append("<ul align='left' id="+un+"> \
        ...
        <a  onclick=\"addfolder(3,'"+un+"')\"><i class='fa fa-folder'>&nbsp; Nouveau article</i></a>\
