...
function reporter() {
  ...
  {
    $("#reporter").val(localStorage.reporter);
...
// Enregistre un nouveau reporter
function new_reporter() {
  localStorage.reporter= $("#reporter").val();
  localStorage.langue= $("#langue_reportage").val();
  reporter();
}
...
function tracer_arbo() {
  ...
  //hideshow(false);
  if(object_selected && !jsonObj.configuration.isSelectable)
   {
    ...
    if(!obj[key].estArticle)
      {
        ...
        <span cle="+key+" onmouseout='hidy()' onmouseover='showinfo("+key+",event);' ondragend='organisation_position(); tracer_arbo();' draggable='true' \
      ...
      {
        $("#child" + obj[key].id_parent).append("<li id=" + key + ">\
        <span cle="+key+" onmouseout='hidy()' onmouseover='showinfo("+key+",event);' ondragend='organisation_position(); tracer_arbo();' draggable='true' ondragstart='drag(event,"+key+")' ondrop='drop(event,"+key+","+obj[key].niveau+")' class='select'>\
        <i id=icon"+key+">&nbsp;</i><span class='fa fa-folder'></span><span  id=gras"+key+" > " + obj[key].nom + "</span></span>\
...
function fillhistorique() {
    for (var i = localStorage.length-1; i >= 0; i--)
      if(localStorage.key(i)!="langue" && localStorage.key(i)!="reporter")
