...
$(document).on('click','.select',function() {
    if(configuration.isSelectable)
        { 
          if($(this).hasClass("selected")) select_one($(this).attr("cle"),false); 
          else select_one($(this).attr("cle"),true); 
          tracer_arbo();
        }
...
function select_one(id,val,obj=null) {
  if (obj==null) 
    obj=jsonObj.fils;
  for (key in obj) {
     obj[key].isSelected=false;  
     if(key==id) obj[key].isSelected=val;
     select_one(id,val,obj[key].fils) ;
  }              
}
...
function drop(ev,cible,niv) {
    ev.preventDefault();
    if(!confirm(t("confirmation_deplacement"))) return;
    ...
    if(!obj[key].estArticle)
      {
       ...
       <i  id=icon"+key+">&nbsp;</i> <span cle="+key+" onmouseout='hidy()' onmouseover='showinfo("+key+",event);' ondragend='organisation_position(); tracer_arbo();' draggable='true' \
        ...
        <li><a onclick=\"supprimer_contenu('" + key + "');\"><i class='fa fa-trash'>&nbsp;<span class='i18n' id='m_contxt_vider_contenu'>" +t('m_contxt_vider_contenu')+ "</span></i></a></li>\
         ...
         //si le niveau dépasse le niveau max on cache la possibilité d'ajouter un sous-niveau
          if(obj[key].niveau==NIV_MAX)  
            $("#niveau"+key).hide();
          //s'il a la première position on cache la possibilité de monter (car déjà le 1er)
          if(obj[key].position==1)
            $("#up"+key).hide();
          if(obj[key].isSelected)
              { 
               if(configuration.isSelectable) $('.selected').removeClass("selected");
                $('span:first', $("#"+key)).addClass("selected");
                object_selected=true;
                //obj[key].isSelected=false;
              }
          //gestion d'affichage gras/normal plus/moin enrouler/dérouler
          style_arbo(obj[key],key);
          //traçage d'arbre récursif 
...
// Traçage des documents indépendament car il passe par d'autres critères
...
// Echange les positions entre deux frères
...
function remplir_modale(type, obj) {
    ...
    if (type == "ter") {
        ...
        var person = $("#txt_general_person").val().split(',');
...
function viderModale() {
    $("#rep1, #desc1,#trep1,#descter").val("");
    $("#rep1, #desc1,#trep1,#descter").keyup();
    $("#rep1, #desc1,#trep1,#descter").mouseout();
    $('#person').tagsinput('removeAll');
    $("#descter").val("");
    $("#errorter").hide();;
    //$("#remarquable").attr("checked","false");
    document.getElementById("remarquable").checked = false;
...
function fillhistorique() {
    ...
    // $("#historique > option").sort(function(b,a){
    //         return parseInt(a.text) > parseInt(b.text) ? 1 : -1;
    //         }).remove().appendTo("#historique");
