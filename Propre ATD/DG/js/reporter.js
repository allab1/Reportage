...
function generer_HTML_items_arbo(obj=null) {
  if(obj==null) 
      obj=newobj;
  for(key in obj)
  { var nom=obj[key].nom.toString();
    if(obj[key].erreur)
    {
      $("#child"+obj[key].id_parent).append("<li style='color:red;' class='select' id=" + key + ">" + nom.substring(0,40) + "</li>");
      ready=false;
      continue;
    }
    ...
    // onmouseover='showinfo("+ key +" ,event);' onmouseout='hidy()'
    ...
    else if(obj[key].estArticle)
      {
        ...
        if(!obj[key].description_ter.trim())
            $("#gras"+key).after(" <span class=badge >1</span>");
  ...
  if(ev.shiftKey || ev.altKey || ev.ctrlKey) $("#fond").css("transition-delay","0s");
    ...
    if(elem==1) 
      { var vide='<i class="fa fa-warning"></i>';
        $("#bref1").html("Versement : <span class='bref'>"+(jsonObj.versement|| vide)+"</span>");
        $("#bref2").html("Depositaire : <span class='bref'>"+(jsonObj.depositaire|| vide)+"</span>");
    ...
    if(tmp.estArticle)
    {
      ...
      $("#bref4").html("Description : <span class='bref'>"+(tmp.description_ter.substring(0,80)+"..."|| vide)+"</span>");
...
function viderModale() {
    ...
    $("#tdesc1, #desc1, #trep1, #descter, #rep1").val("");
    $("#tdesc1, #desc1, #trep1, #descter, #rep1").keyup();
    $("#tdesc1, #desc1, #trep1, #descter, #rep1").mouseout();
...
function creer_reportage() {
  jsonObj.versement = $("#versement").val();   
  jsonObj.depositaire = $("#depositaire").val(); 
...
function description_generale() {
    ...
    if(!$("#depositaire").val())               
      {
        $("#depositaire").css("color","red"); erreur=true; 
      }
      else $("#depositaire").css("color","");
    ...
    if(!$("#versement").val())                    
      {
        $("#versement").css("color","red"); erreur=true; 
      }
        else $("#versement").css("color","");
    ...
    if(erreur) return;
        ...
        $("#errordesc").hide();
        $("#hidedesc").click();
        $("#home").text($("#versement").val());
        $("#gras1").text($("#versement").val());
        $("#new_report").text($("#versement").val());
...
$("#sauvegarder").click(function() {
    ...
    var date = new Date();
    var key=formatDate(date)+"_"+restrict($("#versement").val());
...
function isready()
{
  if(!ready)
  {
    alert(t("erreur_arbo_pas_de_modif"));
...
function progress()
{
  ...
  nbarticle_rempli=0;
  valider_projet();
  if(jsonObj.versement.trim()) { nbarticle_rempli+=3; }
...
function toast(a = "test", b = "test") {
  ...
  if(!jsonObj.versement) { alert(t("projet_non_complet")); return; }
  ...
  jsonObj.arborescence=newobj;
    ...
    a.download = formatDate()+jsonObj.versement+".json";
    a.href = url;
    a.textContent = formatDate()+jsonObj.versement+".json";
}
